...
class Maze:
	def __init__(self, mazeLayer):
		...
		for y in xrange(RESOLUTION_Y/8):
			pygame.draw.line(self.mLayer, (255,255,255,255), (0, y*8), (RESOLUTION_X, y*8))
			for x in xrange(RESOLUTION_X/8):
				self.mazeArray.append(0)
				if (y==0):
					pygame.draw.line(self.mLayer, (255,255,255,255), (x*8,0), (x*8, RESOLUTION_Y))
		...
		if self.state == 'create':
			...
			while(moved == False):
				...
				if len(neighbors) > 0:
					...
					pygame.draw.rect(self.sLayer, (random.randint(0,50),random.randint(50,255),random.randint(50,255),255), Rect(dx+1,dy+1,5,5))
					self.mazeArray[self.currentCell] |= direction << 0
					self.cellStack.append(self.currentCell)
					self.currentCell = nidx
					moved = True
				else:
					pygame.draw.rect(self.sLayer, (255,0,0,255), Rect((x*8)+1,(y*8)+1,5,5))
...
def main():
	...
	displayScreen = pygame.display.set_mode((RESOLUTION_X,RESOLUTION_Y), RESIZABLE)
